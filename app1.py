...
def get_district_data(district_name):
    district_name = ''.join(
        c for c in unicodedata.normalize('NFD', district_name) if not unicodedata.combining(c)).upper()
                ...
                if st.button("Fetch Data"):
                    for city in selected_cities:
                        ...
                        if district_data:
                            ...
                            st.write(f"No district data found for {city}. Using city name as search query.")
                            search_queries = [city]
                        ...
                        if sub_c:
                            with st.spinner(f"Scraping data for {city}..."):
                                ...
                                if combined_results:
                                  st.write(f"Found {len(combined_results)} places in {city}.")
                                  df = pd.DataFrame(combined_results)
                                  st.dataframe(df)

                                  # Generate individual CSV file for each city
                                  csv = df.to_csv(index=False).encode('utf-8')
                                  st.download_button(
                                      label=f"Download data for {city} as CSV",
                                      data=csv,
